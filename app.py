...
# Keep these values the same as setup_database.py.
DB_CONFIG = {
    "host": "127.0.0.1",
    "port": 3306,
    "user": "leave_user",
    "password": "leave_password",
    "database": "leave_tracker_db",
}


def get_connection():
    """Open a MySQL connection for one operation."""
    return mysql.connector.connect(**DB_CONFIG)


def fetch_all(sql, values=()):
    """Run SELECT and return rows as dictionaries."""
...
def execute(sql, values=()):
    """Run INSERT, UPDATE, or DELETE and commit the change."""
    connection = get_connection()
...
def leave_types():
    ...
    rows = fetch_all(
        """
        SELECT id, name, days_allowed, description
        FROM leave_types
        ORDER BY id DESC
        """
    )
...
def add_leave_type():
    ...
    if request.method == "POST":
        ...
        if not name or not days_allowed.isdigit() or not description:
            flash(
                "Enter name, numeric days allowed, and description.",
                "error",
            )
...
def delete_leave_type(leave_type_id):
    ...
    execute(
        "DELETE FROM leave_types WHERE id = %s",
        (leave_type_id,),
    )
...
def api_add_leave_type():
    ...
    if (
        not name
        or not isinstance(days_allowed, int)
        or not description
    ):
        return jsonify(
            error="name, integer days_allowed, and description are required"
        ), 400
...
def api_update_leave_type(leave_type_id):
    ...
    if not fetch_one(
        "SELECT id FROM leave_types WHERE id = %s",
        (leave_type_id,),
    ):
    ...
    execute(
        """
        UPDATE leave_types
        SET name=%s, days_allowed=%s, description=%s
        WHERE id=%s
        """,
...
def add_leave_request():
    ...
    if request.method == "POST":
        ...
        if (
            ...
            or not leave_type_id.isdigit()
            or not leave_date
        ):
            flash(
                "Choose employee, leave type, and date.",
                "error",
            )
...
def api_add_leave_request():
    ...
    if (
        ...
        or not isinstance(leave_type_id, int)
        or not leave_date
    ):
        return jsonify(
            error=(
                "integer employee_id, integer leave_type_id, "
                "and leave_date are required"
            )
        ), 400
...
def api_delete_leave_request(request_id):
    ...
    if not fetch_one(
        "SELECT id FROM leave_requests WHERE id=%s",
        (request_id,),
    ):
